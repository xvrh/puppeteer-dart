...
class RuntimeManager {
  ...
  final StreamController<ExecutionContextDescription> _executionContextCreated =
      new StreamController<ExecutionContextDescription>.broadcast();

  /// Issued when new execution context is created.
  Stream<ExecutionContextDescription> get onExecutionContextCreated =>
      _executionContextCreated.stream;

  final StreamController<ExecutionContextId> _executionContextDestroyed =
      new StreamController<ExecutionContextId>.broadcast();

  /// Issued when execution context is destroyed.
  Stream<ExecutionContextId> get onExecutionContextDestroyed =>
      _executionContextDestroyed.stream;

  final StreamController _executionContextsCleared =
      new StreamController.broadcast();

  /// Issued when all executionContexts were cleared in browser
  Stream get onExecutionContextsCleared => _executionContextsCleared.stream;

  final StreamController<ExceptionThrownResult> _exceptionThrown =
      new StreamController<ExceptionThrownResult>.broadcast();

  /// Issued when exception was thrown and unhandled.
  Stream<ExceptionThrownResult> get onExceptionThrown =>
      _exceptionThrown.stream;

  final StreamController<ExceptionRevokedResult> _exceptionRevoked =
      new StreamController<ExceptionRevokedResult>.broadcast();

  /// Issued when unhandled exception was revoked.
  Stream<ExceptionRevokedResult> get onExceptionRevoked =>
      _exceptionRevoked.stream;

  final StreamController<ConsoleAPICalledResult> _consoleAPICalled =
      new StreamController<ConsoleAPICalledResult>.broadcast();

  /// Issued when console API was called.
  Stream<ConsoleAPICalledResult> get onConsoleAPICalled =>
      _consoleAPICalled.stream;

  final StreamController<InspectRequestedResult> _inspectRequested =
      new StreamController<InspectRequestedResult>.broadcast();

  /// Issued when object should be inspected (for example, as a result of inspect() command line API call).
  Stream<InspectRequestedResult> get onInspectRequested =>
      _inspectRequested.stream;
  ...
  Future<EvaluateResult> evaluate(
    ...
    await _client.send('Runtime.evaluate', parameters);
  }
  ...
  Future<AwaitPromiseResult> awaitPromise(
    ...
    await _client.send('Runtime.awaitPromise', parameters);
  }
  ...
  Future<CallFunctionOnResult> callFunctionOn(
    ...
    await _client.send('Runtime.callFunctionOn', parameters);
  }
  ...
  Future<GetPropertiesResult> getProperties(
    ...
    await _client.send('Runtime.getProperties', parameters);
  }
  ...
  Future<CompileScriptResult> compileScript(
    ...
    await _client.send('Runtime.compileScript', parameters);
  }
  ...
  Future<RunScriptResult> runScript(
    ...
    await _client.send('Runtime.runScript', parameters);
  }
}

class ExceptionThrownResult {
  /// Timestamp of the exception.
  final Timestamp timestamp;

  final ExceptionDetails exceptionDetails;

  ExceptionThrownResult({
    @required this.timestamp,
    @required this.exceptionDetails,
  });

  factory ExceptionThrownResult.fromJson(Map json) {
    return new ExceptionThrownResult(
...
class ExceptionRevokedResult {
  ...
  ExceptionRevokedResult({
    @required this.reason,
    @required this.exceptionId,
  });

  factory ExceptionRevokedResult.fromJson(Map json) {
    return new ExceptionRevokedResult(
      reason: json['reason'],
...
class ConsoleAPICalledResult {
  ...
  factory ConsoleAPICalledResult.fromJson(Map json) {
    return new ConsoleAPICalledResult(
      type: json['type'],
...
class InspectRequestedResult {
  final RemoteObject object;

  final Map hints;

  InspectRequestedResult({
    @required this.object,
    @required this.hints,
  });

  factory InspectRequestedResult.fromJson(Map json) {
    return new InspectRequestedResult(
...
class UnserializableValue {
  ...
  static const UnserializableValue infinity =
      const UnserializableValue._('-Infinity');
  static const UnserializableValue zero = const UnserializableValue._('-0');
