  ...
  group('Puppeteer', () {
    group('Browser.disconnect', () {
      test('should reject navigation when browser closes', () async {
        ...
        try {
          var remote = await puppeteer.connect(
    ...
    group('Puppeteer.launch', () {
      ...
      test('userDataDir option', () async {
        ...
        expect(
          userDataDir.listSync(recursive: true),
      ...
      test('userDataDir option should restore state', () async {
        ...
        try {
          ...
          var browser2 = await puppeteer.launch(
            userDataDir: userDataDir.absolute.path,
          );
      ...
      test('userDataDir option should restore cookies', () async {
        ...
        try {
          ...
          await page2.goto(server.emptyPage);
          expect(
            await page2.evaluate('() => document.cookie'),
    ...
    group('Puppeteer.connect', () {
      test(
        'should be able to connect multiple times to the same browser',
        () async {
      ...
      test(
        'should be able to connect to a browser with no page targets',
        () async {
          var originalBrowser = await puppeteer.launch();
    ...
    test('should be able to connect to the same page simultaneously', () async {
      ...
      var browserTwo = await puppeteer.connect(
  ...
  group('Browser.Events.disconnected', () {
    test(
      ...
      () async {
        var originalBrowser = await puppeteer.launch();
        var browserWSEndpoint = originalBrowser.wsEndpoint;
        var remoteBrowser1 = await puppeteer.connect(
          browserWsEndpoint: browserWSEndpoint,
        );
        var remoteBrowser2 = await puppeteer.connect(
          browserWsEndpoint: browserWSEndpoint,
        );
        ...
        originalBrowser.disconnected.asStream().listen(
          (_) => ++disconnectedOriginal,
        );
        remoteBrowser1.disconnected.asStream().listen(
          (_) => ++disconnectedRemote1,
        );
        remoteBrowser2.disconnected.asStream().listen(
          (_) => ++disconnectedRemote2,
        );
