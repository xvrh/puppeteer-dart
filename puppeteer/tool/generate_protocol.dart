...
main() {
  ...
  var libPath =
      Platform.script.resolve('../../chrome_dev_tools/lib').toFilePath();

  var targetDir = Directory(libPath);
  for (var fileToDelete in targetDir.listSync().whereType<File>().where(
      (file) =>
          file.path.endsWith('.dart') &&
          !file.path.endsWith('chrome_dev_tools.dart'))) {
...
''');
  ...
  _writeDartFile(
      p.join(libPath, 'dev_tools.dart'), tabBuffer.toString());
}
